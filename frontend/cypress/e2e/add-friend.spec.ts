describe('Ajout d\'un ami via le code ami', () => {
    it('Connecte un user, accède à la modal d\'ajout et envoie la demande', () => {
        ...
        cy.contains('Amis', { timeout: 12000 }).should('be.visible').click()

        // 6. Clique sur le bouton "Ajouter" dans la vue amis
        cy.contains('Ajouter', { timeout: 12000 }).should('be.visible').click()

        // 7. La modal d’ajout d’ami doit apparaître
        cy.contains('Ajouter des amis', { timeout: 12000 }).should('be.visible')
        ...
        cy.contains('Demande d’ami envoyée avec succès !', { timeout: 12000 }).should('be.visible')
