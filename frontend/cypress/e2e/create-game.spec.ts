describe('Création d’une partie de jeu en ligne', () => {
    it('userClassique crée une partie, vérifie la présence dans le lobby', () => {
        ...
        cy.get('[data-testid="sidebar-main"]', { timeout: 12000 }).within(() => {
            cy.get('button[title="Jeux en ligne"]', { timeout: 8000 }).click()
        })

        // 4. Vérifie le titre "Jeux en ligne"
        cy.contains('h1', 'Jeux en ligne', { timeout: 8000 }).should('be.visible')

        // 5. Clique sur "Créer une partie"
        cy.contains('button', 'Créer une partie', { timeout: 8000 }).click()
        ...
        cy.contains('h1', 'Jeux en ligne', { timeout: 8000 }).should('be.visible')
        cy.contains('.text-lg.font-bold', roomName, { timeout: 8000 }).should('exist')
        cy.contains('.text-sm.text-gray-300', 'userClassique', { timeout: 8000 }).should('exist')
    })
})
