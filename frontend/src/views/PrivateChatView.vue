<template>
    ...
    <div v-else-if="room" class="flex flex-col h-full p-6 text-white">
        <PrivateChatHeader v-if="enhancedOtherMember" :other-member="enhancedOtherMember"
            @open-friend-modal="handleOpenFriendModal" />
...
import PrivateChatHeader from '@/components/chat/PrivateChatHeader.vue'
...
const isReady = computed(() =>
    ...
    authStore.user &&
    roomStore.privateRoomsList.length > 0
)

const room = computed(() =>
    roomStore.privateRoomsList.find((r) => r.id === roomId.value)
)
...
const otherMember = computed(() => {
    return room.value?.members.find(
        (m) => m.member.friendCode !== authStore.user?.friendCode
    )
})

const enhancedOtherMember = computed(() => {
    if (!otherMember.value) return null

    return {
        ...otherMember.value,
        member: {
            ...otherMember.value.member,
            isOnline: userStatusStore.isOnline(otherMember.value.member.friendCode)
        }
    } as RoomMember
})
...
function handleOpenFriendModal() {
    console.log('Ouvrir la modale de gestion d’amitié')
}
</script>
